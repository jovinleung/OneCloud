...
platform_do_upgrade() {
	...
	# Check /tmp space (need ~rootfs size + 1MB header; streaming avoids full raw image copy)
	local tmp_avail=$(df -k /tmp | tail -1 | awk '{print $4}')
	echo "platform_do_upgrade: /tmp available: ${tmp_avail}KB"
	if [ "$tmp_avail" -lt 550000 ]; then
		echo "ERROR: Not enough space in /tmp (need 550MB, have ${tmp_avail}KB)"
		return 1
	fi
